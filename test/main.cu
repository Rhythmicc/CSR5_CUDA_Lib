...
int call_anonymouslib(char *filename, int m, int n, int nnzA,
                  ...
                  VALUE_TYPE *x, VALUE_TYPE *y, VALUE_TYPE alpha, double*gflops, double*cvt_time, int device_id)
{
    int err = 0;
    ...
    cudaMemcpy(d_csrColIdxA, csrColIdxA, nnzA  * sizeof(int),   cudaMemcpyHostToDevice);
    cudaMemcpy(d_csrValA,    csrValA,    nnzA  * sizeof(VALUE_TYPE),   cudaMemcpyHostToDevice);

    // Vector x
    cudaMalloc((void **)&d_x, n * sizeof(VALUE_TYPE));
    cudaMemcpy(d_x, x, n * sizeof(VALUE_TYPE), cudaMemcpyHostToDevice);
    ...
    double CSR5Spmv_csr_csr5_time = asCSR5_timer.stop();
    cout << "CSR->CSR5 time = " << CSR5Spmv_csr_csr5_time << " ms." << endl;
    *cvt_time = CSR5Spmv_csr_csr5_time;
    //cout << "asCSR5 err = " << err << endl;

    // check correctness by running 1 time
    err = A.spmv(alpha, d_y);
    //cout << "spmv err = " << err << endl;
    cudaMemcpy(y, d_y, m * sizeof(VALUE_TYPE), cudaMemcpyDeviceToHost);
...
int main(int argc, char ** argv)
{
    ...
    // launch compute
    double gflops, cvt_time;
    call_anonymouslib(filename, m, n, nnzA, csrRowPtrA, csrColIdxA, csrValA, x, y, alpha, &gflops, &cvt_time, device_id);
