...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::asCSR()
{
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR5)
    {
        ...
        cudaFree(_csr5_partition_pointer);
        cudaFree(_csr5_partition_descriptor);
        cudaFree(_temp_calibrator);
        cudaFree(_csr5_partition_descriptor_offset_pointer);
        if (_num_offsets) cudaFree(_csr5_partition_descriptor_offset);
...
int anonymouslibHandle<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT>::asCSR5()
{
    ...
    if (_format == ANONYMOUSLIB_FORMAT_CSR)
    {
        ...
        cudaMalloc((void **)&_csr5_partition_pointer, (_p + 1) * sizeof(ANONYMOUSLIB_UIT));

        cudaMalloc((void **)&_csr5_partition_descriptor, _p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(ANONYMOUSLIB_UIT));
        cudaMemset(_csr5_partition_descriptor, 0, _p * ANONYMOUSLIB_CSR5_OMEGA * _num_packet * sizeof(ANONYMOUSLIB_UIT));

        cudaMalloc((void **)&_temp_calibrator, _p * sizeof(ANONYMOUSLIB_VT));
        cudaMemset(_temp_calibrator, 0, _p * sizeof(ANONYMOUSLIB_VT));

        cudaMalloc((void **)&_csr5_partition_descriptor_offset_pointer, (_p + 1) * sizeof(ANONYMOUSLIB_IT));
        cudaMemset(_csr5_partition_descriptor_offset_pointer, 0, (_p + 1) * sizeof(ANONYMOUSLIB_IT));
        malloc_time += malloc_timer.stop();

        // convert csr data to csr5 data (3 steps)
        // step 1. generate partition pointer
        tile_ptr_timer.start();
        err = generate_partition_pointer(_csr5_sigma, _p, _m, _nnz, _csr5_partition_pointer, _csr_row_pointer);
        ...
        cudaMemcpy(&tail, &_csr5_partition_pointer[_p-1], sizeof(ANONYMOUSLIB_UIT),   cudaMemcpyDeviceToHost);
        ...
        if (_num_offsets)
        {
            ...
            malloc_timer.start();
            cudaMalloc((void **)&_csr5_partition_descriptor_offset, _num_offsets * sizeof(ANONYMOUSLIB_IT));
            malloc_time += malloc_timer.stop();
